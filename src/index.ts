...
async function main(): Promise<void> {
  const argv = await yargs(hideBin(process.argv))
    ...
    .option("enforce-budget", {
      ...
      default: false,
      description:
        "Block calls when budget is exceeded (requires --budget-alert)",
    })
    ...
    .option("http-port", {
      type: "number",
      description:
        "Start in HTTP mode using Streamable HTTP transport on this port",
    })
    .check((argv) => {
  ...
  const serverConfig = {
    dbPath: argv.db as string,
    defaultModel: argv["default-model"] as string,
    budgetAlert: argv["budget-alert"] as number | undefined,
    enforceBudget: argv["enforce-budget"] as boolean,
    pricingTablePath: argv["pricing-table"] as string | undefined,
  };

  const httpPort = argv["http-port"] as number | undefined;
