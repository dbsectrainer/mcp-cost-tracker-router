...
export function generateChargebackReport(
  ...
  if (groupBy === "project") {
    ...
    `,
      ...
      .all(fromTs, toTs) as { project_name: string }[];

    const groups: ChargebackGroup[] = projectNames.map(({ project_name }) => {
      const totRow = db
        .prepare(
          `
        SELECT
      ...
      `,
        ...
        .get(project_name, fromTs, toTs) as {
        cost_usd: number;
        sessions: number;
      };
      ...
      `,
        ...
        .all(project_name, fromTs, toTs) as ToolBreakdown[];
    ...
    `,
      ...
      .all(fromTs, toTs) as { session_id: string }[];

    const groups: ChargebackGroup[] = sessionRows.map(({ session_id }) => {
      const totRow = db
        .prepare(
          `
        SELECT
      ...
      `,
        ...
        .all(session_id, fromTs, toTs) as ToolBreakdown[];
...
export function chargebackToCSV(report: ChargebackReport): string {
  const lines: string[] = ["group_name,cost_usd,sessions,tool,tool_cost"];
