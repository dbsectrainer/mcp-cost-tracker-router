...
export function handleGetSessionCost(
  ...
  const budgetThreshold =
    session?.budget_threshold_usd ?? state.budgetThresholdUsd;
  const budgetRemaining =
    budgetThreshold !== null ? budgetThreshold - totals.total_cost_usd : null;
...
export function handleGetToolCosts(
  db: Database.Database,
  state: SessionState,
): object {
...
export function handleResetSession(
  db: Database.Database,
  state: SessionState,
): { newSessionId: string; result: object } {
  // End current session
  ...
  createSession(
    ...
    newSessionId,
    state.model,
    state.budgetThresholdUsd ?? undefined,
  );
...
export function handleRecordUsage(
  db: Database.Database,
  state: SessionState,
  ...
  const budgetWarning =
    budgetThreshold !== null && totals.total_cost_usd >= budgetThreshold
  ...
  let budgetWarning80pct: string | null = null;
