...
type TaskType = "simple" | "reasoning" | "complex";
...
function modelExceedsConstraint(
  model: string,
  maxCostUsd: number,
...
export function handleSuggestModelRouting(
  ...
  const taskReasoningMap: Record<TaskType, string> = {
    simple:
      "Task appears to be a simple classification or Q&A query. claude-haiku-4-5 provides fast, cost-efficient responses for straightforward tasks.",
    reasoning:
      "Task involves code generation or multi-step reasoning. claude-sonnet-4-6 offers the best balance of capability and cost for these workloads.",
    complex:
      "Task requires deep analysis or handling of large context. claude-opus-4-6 delivers the highest capability for complex, context-heavy tasks.",
  };
