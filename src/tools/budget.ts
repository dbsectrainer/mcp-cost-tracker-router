...
export function handleSetBudgetAlert(
  db: Database.Database,
  state: SessionState,
  params: { threshold_usd: number },
): { updatedState: Pick<SessionState, "budgetThresholdUsd">; result: object } {
  if (params.threshold_usd <= 0) {
    throw new McpError(
      ErrorCode.InvalidParams,
      "threshold_usd must be a positive number greater than 0",
    );
  }
  ...
  try {
    ...
    throw new McpError(
      ...
      `Failed to update budget threshold: ${String(err)}`,
    );
  }
