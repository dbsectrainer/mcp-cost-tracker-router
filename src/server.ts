...
import type { SessionState } from "./tools/session.js";
...
import type { RoutingParams } from "./tools/routing.js";
import { handleEstimateWorkflowCost } from "./tools/estimation.js";
import type { WorkflowStep } from "./tools/estimation.js";
...
function makeTextContent(result: object): {
...
export function createServer(config?: ServerConfig): Server {
  ...
  const pricingTable: PricingTable = loadPricingTable(
    resolvedConfig.pricingTablePath,
  );
  ...
  server.setRequestHandler(GetPromptRequestSchema, async (request) => {
    ...
    const focus =
      (request.params.arguments?.["focus"] as string | undefined) ?? "session";
    ...
    if (
      ...
      if (totals.total_cost_usd >= state.budgetThresholdUsd) {
        ...
        const blockedEntry: AuditEntry = {
          ...
          tool_name: String(
            (safeArgs as Record<string, unknown>)["tool_name"] ?? "unknown",
          ),
          tokens:
            Number((safeArgs as Record<string, unknown>)["input_tokens"] ?? 0) +
            Number((safeArgs as Record<string, unknown>)["output_tokens"] ?? 0),
          cost_usd: 0,
    ...
    switch (name) {
      case "get_session_cost": {
        ...
        await tryNotify(
          ...
          "info",
          "cost-tracker-router",
          `Budget check: session=${state.sessionId} cost=$${(result as Record<string, unknown>)["total_cost_usd"]} threshold=${state.budgetThresholdUsd ?? "none"}`,
        );
      ...
      case "get_spend_history": {
        const period = safeArgs["period"] as
          | "day"
          | "week"
          | "month"
          | undefined;
        ...
        await tryNotify(
          ...
          "info",
          "cost-tracker-router",
          `Spend aggregated: period=${period ?? "week"} total_cost=$${(result as Record<string, unknown>)["total_cost_usd"]}`,
        );
        return makeTextContent(result);
      }
      ...
      case "record_usage": {
        ...
        const costUsd = result["cost_usd"] as number;
      ...
      case "suggest_model_routing": {
        ...
        const constraints = safeArgs["constraints"] as
          | { max_cost_usd?: number }
          | undefined;
      ...
      case "estimate_workflow_cost": {
        ...
        const steps: WorkflowStep[] = stepsRaw.map((s, i) => {
          ...
          return {
            tool_name: step["tool_name"] as string,
            estimated_input_tokens: step["estimated_input_tokens"] as number,
            estimated_output_tokens: step["estimated_output_tokens"] as number,
            model:
      ...
      case "export_budget_audit": {
        const fromDate = safeArgs["from_date"] as string | undefined;
        const toDate = safeArgs["to_date"] as string | undefined;
        const auditFormat =
          (safeArgs["format"] as string | undefined) ?? "json";
      ...
      case "set_project": {
        ...
        const budgetUsd = safeArgs["budget_usd"] as number | undefined;
      ...
      case "get_project_costs": {
        ...
        const since = safeArgs["since"] as string | undefined;
      ...
      case "export_chargeback": {
        ...
        const groupBy =
          (safeArgs["group_by"] as "project" | "session" | undefined) ??
          "project";
        const cbFormat = (safeArgs["format"] as string | undefined) ?? "json";
...
export async function createMcpServer(config: ServerConfig): Promise<void> {
