...
import { handleGetSessionCost, handleGetToolCosts, handleResetSession, handleRecordUsage, } from "./tools/session.js";
...
import { initProjectTables, tagSession, setProjectBudget, getProjectCosts, } from "./project-allocator.js";
...
export function createServer(config) {
    ...
    try {
        ...
        throw new McpError(ErrorCode.InternalError, `Failed to initialize database: ${String(err)}`);
    }
    const auditLog = new AuditLog();
    const pricingTable = loadPricingTable(resolvedConfig.pricingTablePath);
    const initialSessionId = crypto.randomUUID();
    createSession(db, initialSessionId, resolvedConfig.defaultModel, resolvedConfig.budgetAlert);
    const state = {
        sessionId: initialSessionId,
        model: resolvedConfig.defaultModel,
        budgetThresholdUsd: resolvedConfig.budgetAlert ?? null,
    };
    const server = new Server({ name: "mcp-cost-tracker-router", version: "0.2.0" }, { capabilities: { tools: {}, resources: {}, prompts: {} } });
            ...
            tools: [
                {
                    ...
                    description: "Returns token totals and USD cost estimates for the current session. All costs are estimates; verify with your LLM provider's billing dashboard.",
                ...
                {
                    name: "get_tool_costs",
                    description: "Returns per-tool cost breakdown for the current session, sorted by cost descending. Useful for identifying the most expensive operations. All costs are estimates.",
                    annotations: { readOnlyHint: true },
                    inputSchema: {
                        type: "object",
                        properties: {},
                ...
                },
                {
                    ...
                    description: "Set a budget threshold in USD. A warning is emitted when session costs reach 80% or 100% of this threshold. Use with --enforce-budget to block calls beyond the limit.",
                ...
                {
                    name: "reset_session",
                    description: "Start a new cost-tracking session. Previous session data is retained in history and accessible via get_spend_history.",
                    annotations: { readOnlyHint: false },
                    inputSchema: {
                        type: "object",
                        properties: {},
                ...
                },
                {
                    ...
                    description: "Query historical spend aggregated by time period (day/week/month). Returns breakdown by model and tool across all sessions. All costs are estimates.",
                    annotations: { readOnlyHint: true },
                    inputSchema: {
                        type: "object",
                        properties: {
                ...
                {
                    name: "record_usage",
                    description: "Record token usage for a tool call. Logs LLM API cost estimates into the tracker. Emits a budget warning (notifications/message) if 80% of threshold is reached. Pricing table versions are tracked in CHANGELOG.md.",
                    annotations: { readOnlyHint: false },
                    inputSchema: {
                        type: "object",
                        properties: {
                            ...
                            model: {
                                type: "string",
                                description: "Model used (e.g. claude-sonnet-4-6). Defaults to server default model.",
                            },
                        },
                ...
                },
                {
                    ...
                    description: "Heuristic model recommendation by task type. Analyzes the task description and returns a recommended model with reasoning and estimated cost per 1K tokens. Optionally factors in cost constraints.",
                    annotations: { readOnlyHint: true },
                    inputSchema: {
                        type: "object",
                        properties: {
                            task_description: {
                                type: "string",
                                description: "Description of the task (e.g. 'classify this tweet', 'generate a REST API', 'analyze this 50-page document')",
                            },
                            constraints: {
                                type: "object",
                                properties: {
                                    max_cost_usd: {
                                        type: "number",
                                        description: "Maximum acceptable cost per ~1K input / 500 output tokens in USD",
                                    },
                                },
                                additionalProperties: false,
                            },
                ...
                },
                {
                    ...
                    description: "Pre-run cost estimation for a multi-step workflow. Provide a list of tool steps with estimated token counts and models to get total and per-step cost estimates. Output is clearly labeled as ESTIMATE.",
                    annotations: { readOnlyHint: true },
                    inputSchema: {
                        type: "object",
                        properties: {
                            steps: {
                                ...
                                items: {
                                    ...
                                    properties: {
                                        ...
                                        model: {
                                            type: "string",
                                            description: "Model to use for this step. Defaults to server default.",
                                        },
                                    },
                                    required: [
                                        "tool_name",
                ...
                },
                {
                    ...
                    description: "Generate a single-file HTML spend report with session cost breakdown, historical spend by period, model cost comparison chart (CSS bars), and budget status. No external CDN dependencies.",
                    annotations: { readOnlyHint: true },
                    inputSchema: {
                        type: "object",
                        properties: {},
                ...
                },
                {
                    ...
                    description: "Export audit log of budget enforcement decisions. Optionally filter by date range and format as JSON or CSV.",
                    annotations: { readOnlyHint: true },
                    inputSchema: {
                        type: "object",
                        properties: {
                ...
                {
                    name: "set_project",
                    description: "Create or update a project with an optional budget in USD.",
                    annotations: { readOnlyHint: false },
                    inputSchema: {
                        type: "object",
                        properties: {
                ...
                },
                {
                    ...
                    description: "Get cost report for a project, optionally filtered by a since date.",
                    annotations: { readOnlyHint: true },
                    inputSchema: {
                        type: "object",
                        properties: {
                ...
                {
                    name: "tag_session",
                    description: "Tag the current session with a project name for cost allocation.",
                    annotations: { readOnlyHint: false },
                    inputSchema: {
                        type: "object",
                        properties: {
                ...
                },
                {
                    ...
                    description: "Generate a chargeback report for internal team billing, grouped by project or session.",
                    annotations: { readOnlyHint: true },
                    inputSchema: {
                        type: "object",
                        properties: {
                            ...
                            group_by: {
                                ...
                                description: 'Group by "project" or "session". Defaults to "project".',
                            },
                            format: {
                                type: "string",
                                enum: ["json", "csv"],
                ...
                },
                {
                    ...
                    description: "Check whether a model is allowed for a given task type under the routing policy.",
                    annotations: { readOnlyHint: true },
                    inputSchema: {
                        type: "object",
                        properties: {
            ...
            resources: [
                {
                    ...
                    description: "Current session token totals and USD cost estimate. All costs are estimates.",
    ...
    server.setRequestHandler(ListPromptsRequestSchema, async () => {
        return {
            prompts: [
                {
                    name: "cost-review",
                    description: "Guided workflow cost analysis prompt. Helps review session spending, identify expensive operations, compare models, and decide whether to continue or optimize.",
                    arguments: [
                        {
                            name: "focus",
                            description: 'Optional focus area: "session", "history", "routing", or "optimization"',
                            required: false,
                        },
                    ],
                },
...
Call \`export_spend_report\` to generate a full HTML report for stakeholder review.`,
        ...
        const instruction = focusInstructions[focus] ?? focusInstructions["session"];
        return {
...
Note: All costs displayed are estimates based on token approximations and the pricing table defined in pricing.ts. Actual billed amounts may differ. Pricing table versions are tracked in CHANGELOG.md.`,
    ...
    server.setRequestHandler(CallToolRequestSchema, async (request) => {
        ...
        if (resolvedConfig.enforceBudget &&
            ...
            if (totals.total_cost_usd >= state.budgetThresholdUsd) {
                ...
                const blockedEntry = {
                    ...
                    tokens: Number(safeArgs["input_tokens"] ?? 0) +
                ...
                await tryNotify(server, "warning", "mcp-cost-tracker-router", `Budget enforcement: call blocked. Session cost $${totals.total_cost_usd.toFixed(6)} >= threshold $${state.budgetThresholdUsd.toFixed(6)}.`);
                throw new McpError(ErrorCode.InvalidRequest, `Budget limit of $${state.budgetThresholdUsd.toFixed(6)} exceeded. Current session cost: $${totals.total_cost_usd.toFixed(6)}. Use reset_session to start fresh or increase budget with set_budget_alert.`);
            }
        }
        switch (name) {
            case "get_session_cost": {
                const result = handleGetSessionCost(db, state);
                await tryNotify(server, "info", "cost-tracker-router", `Budget check: session=${state.sessionId} cost=$${result["total_cost_usd"]} threshold=${state.budgetThresholdUsd ?? "none"}`);
            ...
            case "set_budget_alert": {
                ...
                if (typeof thresholdUsd !== "number") {
                    throw new McpError(ErrorCode.InvalidParams, "threshold_usd must be a number");
                }
                const { updatedState, result } = handleSetBudgetAlert(db, state, {
                    threshold_usd: thresholdUsd,
                });
                state.budgetThresholdUsd = updatedState.budgetThresholdUsd;
                await tryNotify(server, "info", "cost-tracker-router", `Budget check: threshold set to $${thresholdUsd.toFixed(6)} for session=${state.sessionId}`);
                return makeTextContent(result);
            }
            case "reset_session": {
            ...
            case "get_spend_history": {
                ...
                if (period !== undefined &&
                    ...
                    throw new McpError(ErrorCode.InvalidParams, 'period must be one of "day", "week", "month"');
                }
                const requestId = crypto.randomUUID();
                const result = await handleGetSpendHistoryWithProgress(db, { period, requestId }, server);
                await tryNotify(server, "info", "cost-tracker-router", `Spend aggregated: period=${period ?? "week"} total_cost=$${result["total_cost_usd"]}`);
                return makeTextContent(result);
            }
            case "record_usage": {
                ...
                if (typeof inputTokens !== "number" || inputTokens < 0) {
                    throw new McpError(ErrorCode.InvalidParams, "input_tokens must be a non-negative number");
                }
                if (typeof outputTokens !== "number" || outputTokens < 0) {
                    throw new McpError(ErrorCode.InvalidParams, "output_tokens must be a non-negative number");
                }
                if (model !== undefined && typeof model !== "string") {
                    throw new McpError(ErrorCode.InvalidParams, "model must be a string if provided");
                }
                ...
                await tryNotify(server, "info", "cost-tracker-router", `Cost tracked: ${effectiveModel} - input=${inputTokens} output=${outputTokens} cost=$${costUsd.toFixed(6)}`);
                // Emit notification/message warning if 80% of budget is consumed
                if (typeof result["budget_warning_80pct"] === "string") {
                    await tryNotify(server, "warning", "mcp-cost-tracker-router", result["budget_warning_80pct"]);
                }
            ...
            case "suggest_model_routing": {
                ...
                if (typeof taskDescription !== "string" ||
                    ...
                    throw new McpError(ErrorCode.InvalidParams, "task_description must be a non-empty string");
                }
                ...
                await tryNotify(server, "info", "cost-tracker-router", `Routing decision made: recommended=${result.recommended_model} task_type=${result.task_type}`);
                return makeTextContent(result);
            }
            case "estimate_workflow_cost": {
                ...
                const steps = stepsRaw.map((s, i) => {
                    ...
                    if (typeof step["tool_name"] !== "string") {
                        throw new McpError(ErrorCode.InvalidParams, `steps[${i}].tool_name must be a string`);
                    }
                    if (typeof step["estimated_input_tokens"] !== "number" ||
                        step["estimated_input_tokens"] < 0) {
                        throw new McpError(ErrorCode.InvalidParams, `steps[${i}].estimated_input_tokens must be a non-negative number`);
                    }
                    if (typeof step["estimated_output_tokens"] !== "number" ||
                        step["estimated_output_tokens"] < 0) {
                        throw new McpError(ErrorCode.InvalidParams, `steps[${i}].estimated_output_tokens must be a non-negative number`);
                    }
                    return {
                ...
                const result = handleEstimateWorkflowCost({ steps }, state, pricingTable);
                return makeTextContent(result);
            }
            ...
            case "export_budget_audit": {
                ...
                if (auditFormat === "csv") {
                    const header = "timestamp,session_id,tool_name,tokens,cost_usd,budget_usd,decision,reason";
                    const rows = entries.map((e) => `"${e.timestamp}","${e.session_id}","${e.tool_name}",${e.tokens},${e.cost_usd},${e.budget_usd ?? ""},"${e.decision}","${e.reason}"`);
            ...
            case "set_project": {
                ...
                if (typeof projectName !== "string" || projectName.trim() === "") {
                    throw new McpError(ErrorCode.InvalidParams, "project_name must be a non-empty string");
                }
            ...
            case "export_chargeback": {
                ...
                if (typeof fromDate !== "string" || typeof toDate !== "string") {
                    throw new McpError(ErrorCode.InvalidParams, "from_date and to_date are required strings");
                }
                const groupBy = safeArgs["group_by"] ??
                    "project";
            ...
            case "check_routing_policy": {
                ...
                if (typeof taskType !== "string" || taskType.trim() === "") {
                    throw new McpError(ErrorCode.InvalidParams, "task_type must be a non-empty string");
                }
                if (typeof model !== "string" || model.trim() === "") {
                    throw new McpError(ErrorCode.InvalidParams, "model must be a non-empty string");
                }
