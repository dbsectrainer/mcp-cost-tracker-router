...
export function updateSessionBudget(db, sessionId, thresholdUsd) {
    const stmt = db.prepare("UPDATE sessions SET budget_threshold_usd = ? WHERE id = ?");
...
export function recordToolUsage(db, usage) {
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `);
    stmt.run(usage.id, usage.session_id, usage.tool_name, usage.model, usage.input_tokens, usage.output_tokens, usage.cost_usd, usage.recorded_at);
}
